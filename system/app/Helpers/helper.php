...
function allUpazillas()
{
    $upazilas = array(
        ...
        array('id' => '460', 'district_id' => '61', 'name' => 'Nakla', 'bn_name' => 'নকলা', 'url' => 'nokla.sherpur.gov.bd'),
...
function findBelongsTo($array, $matching)
{
...
function findHasMany($array, $matching)
{
    $items = [];
    foreach ($array as $item) {
        $is_match = true;
...
function routeDetails()
{
    ...
    foreach ($routeCollection as $value) {
        if ($value->methods()[0] == 'GET' && in_array("auth", $value->action['middleware']) && !in_array("only.ajax", $value->action['middleware'])) {
            $activeRoute = explode('.', $value->getName())[0];
    ...
    $routes = [
        ...
        'activeRoutes' => array_unique($activeRoutes)
    ];
    return $routes;
}
