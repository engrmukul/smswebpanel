                ...
                <div class="card">
                    ...
                    <div class="card-content">
                        <div class="card-body">
                            ...
                            <form class="form form-horizontal" method="post" action="{{ route('email.edit.post', $email->id) }}" enctype="multipart/form-data" id="sendEmailForm">
                                {{ csrf_field() }}
                                <input type="hidden" id="status" name="status" value="{{ $email->status }}" />
                                <div class="form-body px-1 pt-1">
                                        ...
                                        <div class="col-12 input-element {{ $errors->has('domain') ? 'has-error' : '' }}">
                                            <select class="selectdomain form-control custom-select block border-primary" id="domain" name="domain">
                                                ...
                                                @foreach($domains as $domain)
                                                    <option value="{{ $domain->domain }}" {{ ($email->domain == $domain->domain)? 'selected': '' }}>{{ $domain->domain }}</option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="form-group row">
                                        <div class="col-12 input-element {{ $errors->has('subject') ? 'has-error' : '' }}">
                                            <input type="text" placeholder="Subject" class="selectdomain form-control block border-primary" id="subject" name="subject" value="{{ $email->subject }}">
                                    ...
                                    <div class="form-group row">
                                        <div class="col-12 input-element {{ $errors->has('from_email') ? 'has-error' : '' }}">
                                            <input type="text" placeholder="From Email" class="selectdomain form-control block border-primary" id="from_email" name="from_email" value="{{ $email->from_email }}">
                                        ...
                                        <div class="tab-pane active" id="sendEmail" role="tabpanel" aria-labelledby="base-sendEmail">
                                            <div class="form-group row">
                                                <div class="col-12 input-element {{ $errors->has('recipient') ? 'has-error' : '' }}" id="recepientDiv">
                                                    <textarea placeholder="To Email List" id="recipient" name="recipient" class="block border-primary">{{ $email->recipients }}</textarea>
                                                ...
                                                <div class="col-12 input-element {{ $errors->has('group_id') ? 'has-error' : '' }}">
                                                    <div class="form-group row" style="padding-right: 15px; padding-left: 15px;">
                                                        @foreach($Groups as $Group)
                                                            ...
                                                            @php($group_ids = explode(',', $email->group_id))
                                                            <div class="col-xl-4 col-md-3 custom-control custom-checkbox">
                                                                <input type="checkbox" value="{{ $Group->id }}" name="group_id[]" id="users-checkbox{{ $Group->id }}" {{ in_array($Group->id, $group_ids)? 'checked':'' }} onclick="{{ ($countEmail == 0)? 'alert(\'No Email In This Group\'); return false;':'return true;' }}" class="custom-control-input user-checkbox">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-12 input-element {{ $errors->has('message') ? 'has-error' : '' }}">
                                            <textarea id="message" name="message" class="block border-primary" rows="4">{{ $email->message }}</textarea>
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-9 col-form-label custom-control custom-checkbox input-element {{ $errors->has('schedule') ? 'has-error' : '' }}">
                                            <input type="checkbox" value="yes" name="schedule" id="schedule" class="form-control custom-control-input user-checkbox" {{ !empty($email->schedule_time)? 'checked':'' }} onclick="scheduleFunction();">
                                            <label class="custom-control-label" for="schedule">YES</label>
                                            <span class="text-danger">{{ $errors->first('schedule') }}</span>
                                        </div>
                                    </div>

                                    <div class="form-group row" id="scheduleTime" style="display: {{ ($errors->has('schedule_time') or old('schedule_time') != '' or !empty($email->schedule_time))  ? '' : 'none' }}">
                                        ...
                                        <div class="col-12 input-element {{ $errors->has('schedule_time') ? 'has-error' : '' }}">
                                            <div class="input-group">
                                                <input type="text" autocomplete="off" id="schedule_time" class="form-control border-primary" value="{{ $email->schedule_time }}" name="schedule_time" placeholder="yyyy-mm-dd">
                                    ...
                                    <input name="email_type" id="email_type" type="hidden" value="{{ empty(old('email_type'))? $email->email_type:old('email_type')  }}">
                                </div>
...
@section('pageJS')
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $('#tabMenu a[href="#{{ empty(old('email_type'))? $email->email_type:old('email_type') }}"]').tab('show');
