                            ...
                            @if (empty(Auth::user()->reseller_id))
                                <div class="form-group justify-content-end row">
                                    <div class="col-md-3 mb-1 justify-content-end row mr-1">
                                        <button link="{{ route('config.domain.add') }}" class="btn btn-primary btn-md submit"><i class="d-md-none d-block feather icon-plus"></i>
                                            <span class="d-md-block d-none">Add New Domain</span>
                                        </button>
                                    </div>
                                </div>
                            @endif
...
@section('pageJS')
    ...
    <script>
        ...
        $(document).on("click", "#change_status_menu", function () {
            var route = '{{ route('config.domain.status', ':id') }}';
